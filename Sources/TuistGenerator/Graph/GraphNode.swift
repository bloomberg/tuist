...
class TargetNode: GraphNode {
    ...
    static func read(name: String,
        ...
        let dependencies: [GraphNode] = try target.dependencies.map {
            try self.readDependency(path: path,
                                    name: name,
                                    dependency: $0,
                                    cache: cache,
                                    circularDetector: circularDetector,
    ...
    static func readDependency(path: AbsolutePath,
        ...
        if type == "target" {
            ...
            let circularFrom = GraphCircularDetectorNode(path: path, name: name)
            let name = try dependency.target.valueOrThrow(error)
            let projectRelativePath = try dependency.path.valueOrThrow(error)
            let projectPath = path.appending(projectRelativePath)
            let circularTo = GraphCircularDetectorNode(path: projectPath, name: name)
            try circularDetector.start(from: circularFrom, to: circularTo)
            return try TargetNode.read(name: name,
                                       path: projectPath, cache: cache,
                                       circularDetector: circularDetector,
                                       modelLoader: modelLoader)
        } else if type == "framework" {
            let frameworkPath = try dependency.path.valueOrThrow(error)
            return try FrameworkNode.parse(projectPath: path,
                                           path: frameworkPath,
                                           cache: cache)
        } else if type == "library" {
            let libraryPath = try dependency.path.valueOrThrow(error)
            let publicHeaders = try dependency.publicHeaders.valueOrThrow(error)
            let swiftModuleMap = dependency.swiftModuleMap
...
class LibraryNode: PrecompiledNode {
    ...
    static func parse(publicHeaders: RelativePath,
        ...
        let publicHeadersRelativePath = publicHeaders
        ...
        if let swiftModuleMapRelativePathString = swiftModuleMap {
