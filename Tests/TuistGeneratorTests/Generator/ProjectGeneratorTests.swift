...
final class ProjectGeneratorTests: TuistUnitTestCase {
    ...
    func test_generate_scheme() throws {
        ...
        let sharedScheme = Scheme.test(name: "Target-Scheme", shared: true, buildAction: BuildAction(targets: [TargetReference(projectPath: nil, name: "Target")]))
    ...
    func test_generate_local_scheme() throws {
        ...
        let temporaryPath = try self.temporaryPath()
        let target = Target.test(name: "Target", platform: .iOS, product: .framework)
        let localScheme = Scheme.test(name: "Target-Local", shared: false, buildAction: BuildAction(targets: [TargetReference(projectPath: nil, name: "Target")]))
