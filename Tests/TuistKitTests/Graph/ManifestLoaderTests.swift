...
final class ManifestLoaderErrorTests: XCTestCase {
    func test_description() {
        ...
        XCTAssertEqual(ManifestLoaderError.invalidYaml(AbsolutePath("/test/")).description, "Invalid yaml at path /test. The root element should be a dictionary")
        XCTAssertEqual(ManifestLoaderError.manifestNotFound(.project, AbsolutePath("/test/")).description, "Project not found at path '/test'")
        XCTAssertEqual(ManifestLoaderError.missingFile(AbsolutePath("/test/")).description, "Couldn't find file at path '/test'")
        XCTAssertEqual(ManifestLoaderError.setupNotFound(AbsolutePath("/test/")).description, "Setup.swift not found at path '/test'")
    }
    ...
    func test_type() {
        ...
        XCTAssertEqual(ManifestLoaderError.invalidYaml(AbsolutePath("/test/")).type, .abort)
        XCTAssertEqual(ManifestLoaderError.manifestNotFound(.project, AbsolutePath("/test/")).type, .abort)
        XCTAssertEqual(ManifestLoaderError.missingFile(AbsolutePath("/test/")).type, .abort)
        XCTAssertEqual(ManifestLoaderError.setupNotFound(AbsolutePath("/test/")).description, "Setup.swift not found at path '/test'")
    }
}
...
final class ManifestLoaderTests: XCTestCase {
    ...
    override func setUp() {
        ...
        subject = ManifestLoader(fileHandler: fileHandler,
